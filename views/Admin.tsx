...
import { Transaction, PremiumMission } from '../types';
import { useGame } from '../context/GameContext';

const Admin: React.FC = () => {
  const { user, globalUsers: users, setGlobalUsers: setUsers, transactions, setTransactions, settings, setSettings, t } = useGame();
  const [activeTab, setActiveTab] = useState<'users' | 'deposits' | 'withdrawals' | 'settings' | 'premium'>('users');
  const [searchTerm, setSearchTerm] = useState('');
  const [balanceInputs, setBalanceInputs] = useState<Record<string, string>>({});

  // Premium Mission Creation State
  const [newMission, setNewMission] = useState<Partial<PremiumMission>>({
    title: '',
    link: '',
    waitTime: 60,
    reward: 0.1,
  ...
  const createMission = () => {
    if (!newMission.title || !newMission.link || !newMission.logo) {
      alert("Missing data for the mission");
      return;
    }
    const mission: PremiumMission = {
      ...
      waitTime: newMission.waitTime || 60,
      reward: newMission.reward || 0.1,
      maxUsers: newMission.maxUsers || 100,
      logo: newMission.logo,
      completedUserIds: []
    };
  ...
  const processTx = (txId: string, status: 'approved' | 'rejected') => {
    ...
    if (status === 'approved') {
      if (tx.type === 'deposit') {
        ...
        // Find target user to give deposit
          ...
          <div className="bg-zinc-900 border border-zinc-800 p-6 rounded-2xl space-y-4">
            ...
            <div className="grid grid-cols-1 gap-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <AdminInput label={t.missionTime} type="number" value={newMission.waitTime?.toString() || ''} onChange={v => setNewMission({ ...newMission, waitTime: parseInt(v) })} />
                <AdminInput label={t.missionRewardVal} type="number" value={newMission.reward?.toString() || ''} onChange={v => setNewMission({ ...newMission, reward: parseFloat(v) })} />
                <AdminInput label={t.missionUsers} type="number" value={newMission.maxUsers?.toString() || ''} onChange={v => setNewMission({ ...newMission, maxUsers: parseInt(v) })} />
              </div>
          ...
          <div className="bg-zinc-950 border border-zinc-800 p-6 rounded-2xl space-y-6">
            <div className="flex justify-between items-center">
              <div>
                ...
                <p className="text-[9px] text-zinc-600 font-black uppercase">Permitir CWARS &rarr; TON</p>
              </div>
              <button
...
const AdminInput: React.FC<{ label: string, value: string, onChange: (v: string) => void, type?: string }> = ({ label, value, onChange, type = "text" }) => (
  <div className="space-y-1">
    <label className="text-[8px] text-zinc-500 font-black uppercase ml-1">{label}</label>
    <input type={type} value={value} onChange={e => onChange(e.target.value)} className="w-full bg-black border border-zinc-800 p-3 rounded-xl text-xs text-white outline-none focus:border-red-600" />
  </div>
);
